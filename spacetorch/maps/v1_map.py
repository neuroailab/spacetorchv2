...
def write_tc_fits_to_cache(output_dir: Path, fits, responses):
    tc_cache_dir = output_dir / "cache"
    tc_cache_dir.mkdir(exist_ok=True, parents=True)

    generic_utils.write_pickle(tc_cache_dir / "tuning_curve_fits.pkl", fits)
    generic_utils.write_pickle(tc_cache_dir / "tuning_curve_responses.pkl", responses)
...
class V1Map(TissueMap):
    ...
    def _smooth_orientation_tuning_curves(self):
        ...
        cached = tc_fits_from_cache(self.output_dir)
        ...
        if cached is not None and not self.skip_cache:
            ...
            write_tc_fits_to_cache(self.output_dir, fits, self.responses)
