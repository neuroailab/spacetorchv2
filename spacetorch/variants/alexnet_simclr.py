...
import spacetorch.variants.assets.simclr.algorithms as alg
...
from spacetorch.variants.assets.simclr.dataloader import DataLoader, GenericDataset
from spacetorch.variants.assets.simclr.architectures.AlexNet import create_model


class AlexNetSimCLR(BaseArch):
    """
    Implements the SimCLR objective function.
    """
    def __init__(self):
        super().__init__()
    ...
    def set_eval_cfg(self, args):
        self.eval_cfg = argparse.Namespace(**{
            "exp": "ImageNet_LinearClassifiers_ImageNet_SimCLR_AlexNet_Features",
            "evaluate": False,
            "checkpoint": 0,
            "num_workers": 4,
            "cuda": True,
    ...
    def set_model(self, args):
        ...
        self.positions = None  # self._load_positions(args.spatial_loss.position_dir)
...
def main(args, positions, layernames, args_opt):
    exp_config_file = os.path.join('spacetorch/variants/assets/simclr/', args_opt.exp+'.py')
    ...
    algorithm = getattr(alg, config['algorithm_type'])(config, model_wrapper=SpatialAlexNetSimCLR, args=args, positions=positions, layernames=layernames)
...
class SpatialAlexNetSimCLR(torch.nn.Module):
