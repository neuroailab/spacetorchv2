...
from spacetorch.variants.assets.deepcluster.util import AverageMeter, learning_rate_decay, load_model, Logger
...
def main(args):
    ...
    for epoch in range(args.epochs):
        ...
        # train for one epoch
        train(train_loader, model, reglog, criterion, optimizer, epoch, args)

        # evaluate on validation set
        prec1, prec5, loss = validate(val_loader, model, reglog, criterion, args)
...
def train(train_loader, model, reglog, criterion, optimizer, epoch, args):
...
def validate(val_loader, model, reglog, criterion, args):
    batch_time = AverageMeter()
    losses = AverageMeter()
    top1 = AverageMeter()
    top5 = AverageMeter()
    ...
    for i, (input_tensor, target) in enumerate(val_loader):
        ...
        if args.tencrops:
            output_central = output.reshape(bs, ncrops, -1)[: , ncrops // 2 - 1, :]
            output = softmax(output)
            output = torch.squeeze(output.reshape(bs, ncrops, -1).mean(1))
        else:
