...
from spacetorch.variants.assets.rotnet.algorithms import Algorithm
...
class ClassificationModel(Algorithm):
    ...
    def process_batch(self, batch, do_train=True):
        ...
        #************ FORWARD THROUGH NET ***********************
        pred_var, spatial_outputs = self.networks['model'](dataX_var)
        #********************************************************

        #*************** COMPUTE LOSSES *************************
        record = {}
        loss_task = self.criterions['loss'](pred_var, labels_var)
