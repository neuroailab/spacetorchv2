...
import spacetorch.variants.assets.rotnet.utils as utils
import datetime
import logging
...
class Algorithm():
    ...
    def init_all_networks(self):
        ...
        for key, val in networks_defs.items():
            ...
            network = self.init_network(def_file, net_opt, pretrained_path, key)
            if self.model_wrapper is not None:
                self.networks[key] = self.model_wrapper(network, args=self.args, positions=self.positions, layernames=self.layernames)
            else:
    ...
    def load_pretrained(self, network, pretrained_path):
        ...
        for k in list(pretrained_model['network'].keys()):
            # retain only base_encoder up to before the embedding layer
            if k.startswith('model.'):
                # remove prefix
                pretrained_model['network'][k[len("model."):]] = pretrained_model['network'][k]
                # delete renamed or unused k
                del pretrained_model['network'][k]
