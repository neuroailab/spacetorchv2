...
_IMAGENET_DATASET_DIR = '/data2/ynshah/imagenet'
    ...
    def __init__(self, dataset_name, split, random_sized_crop=False,
        ...
        if self.dataset_name=='imagenet':
            ...
            if self.split!='train':
                ...
                transforms_list = [
                    transforms.Resize(256),
                    transforms.CenterCrop(256),  # keep more context for SimCLR to crop from
                    lambda x: np.asarray(x),
                ]
...
class DataLoader(object):
    def __init__(self,
        ...
        self.mean_pix  = self.dataset.mean_pix
        self.std_pix   = self.dataset.std_pix
        ...
        if self.unsupervised:
            ...
            def get_simclr_transform(img_size=224):
                return transforms.Compose([
                    ...
                    transforms.ToTensor(),
                    transforms.Normalize(mean=self.dataset.mean_pix, std=self.dataset.std_pix)
                ])

            # In get_iterator, replace the unsupervised _load_function:
            def _load_function(idx):
                idx = idx % len(self.dataset)
                img0, _ = self.dataset[idx]
                simclr_transform = get_simclr_transform()
                view1 = simclr_transform(img0)
                view2 = simclr_transform(img0)
                return torch.stack([view1, view2], dim=0), torch.LongTensor([0, 1])  # labels unused

            def _collate_fun(batch):
                batch = default_collate(batch)
                batch_size, views, c, h, w = batch[0].size()
                batch[0] = batch[0].view([batch_size * views, c, h, w])
                return batch
