...
class LCNN(BaseArch):
    ...
    def set_eval_model(self, args):
        pool_type = args.name.split("_")[-1]
        print("pool type:", pool_type)
        if pool_type == "mean":
            self.eval_model = load_model(pool_type='mean', kap_kernelsize=0.1, continuous=True, local_conv=False)
        elif pool_type == "gaussian":
            self.eval_model = load_model(pool_type=pool_type, kap_kernelsize=0.05, continuous=True, local_conv=False)
        elif pool_type == "mexicanhat":
            self.eval_model = load_model(pool_type='mexicanhat', kap_kernelsize=1.0, continuous=True, local_conv=False)
        else:
            self.eval_model = resnet18(pretrained=True)
