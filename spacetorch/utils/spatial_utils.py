...
def precompute_neighborhoods(
    ...
    print('\tys:', start_ys.shape, '\tmin:', np.min(start_ys), '\tmax:', np.max(start_ys))
    ...
    print('\tneighborhoods:', len(neighborhoods), '\tmin points:', min([len(x) for x in neighborhoods]), '\tmax points:', max([len(x) for x in neighborhoods]))
    return neighborhoods
...
def place_tokens(
    ...
    print('\tanchors (on the tissue):', anchors.shape)
