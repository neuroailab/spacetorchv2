...
def main():
    ...
    for (_, metric), ax in zip(METRIC_DICT.items(), axs):
        _ = v1_tissue.make_parameter_map(
            ax, metric=metric, linewidths=0, edgecolor=(0, 0, 0, 0.5), rasterized=True, final_s=3
        )
        ax.axis("off")
        cbar = add_sine_colorbar(fig, ax, metric, label=metric.xlabel)
        cbar.ax.tick_params(labelsize=7)
    ...
    plt.savefig(save_dir / f"v1_maps.png", dpi=300, bbox_inches="tight", transparent=True)
